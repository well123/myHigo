...
abstract class WebSocket2{
    ...
    public function run(){
        while(true){
            ...
            $read = $this->sockets;
            $write = $except = NULL;
            array_push($this->heldMessages,array('message'=>1));
            $this->_tick();
            $this->tick();
    ...
    protected function doHandshake($user, $buffer){
        ...
        $subProtocol =
            (isset($headers['sec-websocket-protocol'])) ? $this->processProtocol($headers['sec-websocket-protocol']) :
                "";
    ...
    protected function extractHeaders($message){
        ...
        if($header['length'] == 126){
            ...
            $header['length'] =
                ord($message[2]) * 65536 * 65536 * 65536 * 256 + ord($message[3]) * 65536 * 65536 * 65536 +
                ord($message[4]) * 65536 * 65536 * 256 + ord($message[5]) * 65536 * 65536 +
                ord($message[6]) * 65536 * 256 + ord($message[7]) * 65536 + ord($message[8]) * 256 + ord($message[9]);
    ...
    protected function checkRSVBits($headers,
                                    $user){ // override this method if you are using an extension where the RSV bits are used.
